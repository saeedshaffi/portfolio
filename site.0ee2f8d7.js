...
const careerStops=[
 ...
 {dates:'Feb to Jul 2021',org:'CreditBook',role:'Senior Product Designer',win:'Design system from scratch',detail:'Led a cross-functional team building CreditBook\u2019s design system, lifting product quality by 40%.',stat:'+40%',statLabel:'product quality',how:['Baseline: 45 UI defects per release (Jira)','After design system: 27 defects \u2248 -40%','Method: defect density + 10-point heuristic review']},
 ...
 {dates:'Jan to Dec 2023',org:'Expertlead',role:'Lead Product Designer',win:'Leading design at scale',detail:'Streamlined design ops (+55% team productivity) and shipped an \u201cInterview as a Service\u201d product.',stat:'+55%',statLabel:'team productivity',how:['Baseline: ~9 days design cycle per feature','After new ops: ~4 days \u2248 +55% throughput','Method: cycle-time tracking in Linear across sprints']},
 {dates:'2024, Present',org:'Deel',role:'Senior Product Designer',win:'Client experience, redesigned',detail:'Led the complete redesign of the client experience and shipped reporting and employee payroll processing, lifting client retention and satisfaction.',stat:'Payroll',statLabel:'and reporting shipped to clients',how:['Redesigned the client experience end to end for a more intuitive interface','Shipped reporting and employee payroll processing','Result: higher client retention and positive feedback'],now:true}
];
const careerTimeline=()=>`<section class="page section home-timeline reveal" aria-labelledby="career-timeline-title"><div class="section-head"><div><span class="eyebrow">Career journey</span><h2 id="career-timeline-title">Ten years,<br>six chapters.</h2></div><p>From founding UX practices to designing global payroll at Deel, every stop moved a metric. Tap or hover a number to see how it was measured.</p></div><div class="timeline-track" role="list"><div class="timeline-rail" aria-hidden="true"></div>${careerStops.map((s,i)=>`<article class="timeline-stop${s.now?' timeline-stop-now':''}" role="listitem" tabindex="0" style="--i:${i}"><span class="timeline-dot" aria-hidden="true"></span><span class="timeline-dates">${s.dates}${s.now?'<i class="timeline-live" aria-hidden="true"></i>':''}</span><h3 class="timeline-org">${s.org}</h3><p class="timeline-role">${s.role}</p><p class="timeline-win">${s.win}</p><p class="timeline-detail">${s.detail}</p><p class="timeline-stat"><strong>${s.stat}</strong><span>${s.statLabel}</span></p><span class="timeline-tip" role="tooltip"><b>How this was measured</b>${s.how.map(x=>`<i>${x}</i>`).join('')}</span></article>`).join('')}</div></section>`;
...
function initHomeRotatingWords(){
  ...
  const fit = (track) => {
    ...
    /* min-width, not width: the CSS ghost word (hero.css) already reserves the
       widest word in the live font, so a measurement taken mid-animation or
       before the webfont lands can never shrink the slot below it. */
    track.style.minWidth = Math.ceil(widest) + 4 + 'px';
  };
...
function renderRoute(){
  ...
  if(path==='/about'||path==='/resume-old'||path==='/resume'){
    (function(){const a=document.createElement('a');a.href='assets/SaeedShaffi_Resume.pdf?v=2026-09';a.download='SaeedShaffi_Resume.pdf';document.body.appendChild(a);a.click();a.remove();})();
  ...
  document.body.classList.toggle('case-active',path.startsWith('/case/'));
    ...
    <div class="home-redesign" data-home-redesign>
      ...
      <section class="experience-strip" aria-labelledby="experience-title">
        ...
        <div class="experience-logos" role="list" aria-label="Selected companies and product environments">
          <b role="listitem">Deel</b><b role="listitem">Zand Bank</b><b role="listitem">KFH Jazeel</b><b role="listitem">PriceOye</b><b role="listitem">PayPro</b><b role="listitem">CreditBook</b><b role="listitem">Expertlead</b>
        </div>
      </section>
